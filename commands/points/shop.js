    ...
    execute(msg, args, client) {
        ...
        query(`SELECT * FROM members WHERE member_id = ${msg.member.id}`, data => {
            ...
            for (let row of result) {
                ...
                if (!categoriesObject.find(cat => cat.category === category) && args.length !== 0) return msg.channel.send(`\`${category}\` is not a shop category!`)
                ...
                if (!category) {
                    embed.setTitle(`Shop Categories`)
                    ...
                    embed.setFooter(`To use an item, do: $use (item id) | (${currentPage}/${lastPage})`)
