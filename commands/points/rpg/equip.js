...
module.exports.command = {
    execute(msg, args, amount, client) {
        query(`SELECT * FROM members_rpg WHERE member_id = ${msg.member.id}`, data => {
            ...
            let alreadyEquippedItem

            if (equippedWeapon) { if (equippedWeapon.equipped === true) {
                if (equippedWeapon.name === item_id) {
                    return msg.channel.send(`You already have this item equipped! Unequip with \`$rpg unequip <item id>\``)
                } else {
                    alreadyEquippedItem = equippedWeapon
                }
            } }
