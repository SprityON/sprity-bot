...
module.exports.command = {
    execute(msg, args, amount, client) {
        ...
        // functions //
        function createProfileEmbed(result, member) {
            ...
            .setAuthor(`Profile of ${result.rpg_name} | LVL. ${result.level}`, member.user.avatarURL({dynamic: true}))
            .addField(`Stats (${result.attributes} ATTR.)`, `HP: ${basic_stats_json.health}\nDEF: ${basic_stats_json.defense}\nATT: ${basic_stats_json.attack}`, true)
            ...
            .setFooter(`Spend your attributes with $rpg profile upgrade <attack|defense|health> <amount>`)
            return embed
        }
        ...
        if (msg.mentions.members.first()) {
            ...
            query(`SELECT * FROM members_rpg WHERE member_id = ${mentioned.id}`, data => {
                if (data[0].length === 0) return msg.channel.send(`**${mentioned.user.username}** does not have a RPG profile!`)
                msg.channel.send(createProfileEmbed(data[0][0], mentioned))
            })
        } else {
            query(`SELECT * FROM members_rpg WHERE member_id = ${msg.member.id}`, data => {
                if (!args[1]) {
                    return msg.channel.send(createProfileEmbed(data[0][0], msg.member))
                }
                
                args[1].toLowerCase()
                if (args[1] !== 'upgrade' && args[1] !== 'u') return msg.channel.send(`That is not a correct argument! You either did not mention a member, or typed in a wrong stat.`)
                ...
                } else return msg.channel.send(`${args[2]} is not a stat!\nValid stats are: \`attack\`, \`defense\`, \`health\``)
            })
        }
    }
}